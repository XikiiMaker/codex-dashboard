...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    const esp_lcd_touch_config_t tp_cfg = {
        ...
        .flags = {
            /*
             * 三个标志全 0，也不挂 process_coordinates：坐标一个字都不动，旋转交给 LVGL。
             *
             * 别按 BSP 旧注释去补 800×480 → 1024×600 的缩放。2026-09-13 的串口抓取里
             * raw x 落在 40..974、y 落在 18..546，这块屏的 GT911 原生就是面板的
             * 1024×600；按 800×480 缩放会把坐标放大 1.28 倍再截断，raw 超过 800 的
             * 那半边全被压到同一条边上，表现为「大半屏幕点了没反应」。
             *
             * 竖屏（sw_rotate + LV_DISPLAY_ROTATION_90）下这里曾挂 mirror_x +
             * swap_xy 自己反变换，那是错的，而且错得没有声响：LVGL 9 的
             * indev_pointer_proc() 对每个指针点都会调 lv_display_rotate_point()，
             * ROTATION_90 的规则是 X = ver_res - y - 1、Y = x，它要的是**物理**
             * 1024×600 坐标。驱动先转成竖屏逻辑值、LVGL 再转一遍，两遍 90° 叠起
             * 来等于 X = raw_x - 425、Y = raw_y —— raw_x < 425 的整片算出负数被
             * 丢掉，表现就是「点了没反应」。（CONFIG_LV_USE_LOG 没开，那句
             * "X is ... smaller than zero" 的警告压根不会打出来。）
             *
             * esp_lvgl_port 不会重复转：lvgl_port_disp_rotation_update() 在
             * sw_rotate 为真时第一行就 return，它的 lvgl_port_touchpad_read()
             * 也只是把 esp_lcd_touch 的值原样抄给 data->point。全链路上转坐标的
             * 只有 LVGL 一处。
             *
             * x_max/y_max 仍按物理值填：哪天实测发现某个轴是反的，mirror 要以
             * 它们为基准。
             */
            .swap_xy = 0,
            .mirror_x = 0,
    ...
    if (ret == ESP_OK) {
#if CONFIG_BSP_LCD_TYPE_1024_600
        ESP_LOGI(TAG, "Touch ready: GT911 native 1024x600, coords passed through (LVGL rotates)");
#endif
    }
    return ret;
}
